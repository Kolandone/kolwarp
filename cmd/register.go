...
var registerCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			log.Fatalf("Failed to get wireguard flag: %v", err)
		}
		...
		var wgPrivateKey, wgPeerPublicKey string

		if useWireGuard {
			log.Printf("Registering with WireGuard mode...")

			privKey, pubKey, err := api.GenerateWireGuardKeyPair()
			if err != nil {
				log.Fatalf("Failed to generate WireGuard key pair: %v", err)
			}

			wgPrivateKey = base64.StdEncoding.EncodeToString(privKey)
			wgPeerPublicKey = internal.WGPeerPublicKey

			log.Printf("Enrolling WireGuard device key...")
			...
			config.AppConfig = config.Config{
				...
				WGEndpointV6:   internal.WGDefaultEndpointV6,
			}
		} else {
...
func init() {
	...
	registerCmd.Flags().Bool("wireguard", false, "Register with WireGuard mode instead of MASQUE")
