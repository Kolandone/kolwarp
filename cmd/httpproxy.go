...
var httpProxyCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		useWireGuard, err := cmd.Flags().GetBool("wireguard")
		if err != nil {
			cmd.Printf("Failed to get wireguard flag: %v\n", err)
			return
		}
		...
		if useWireGuard {
			if !config.AppConfig.HasWireGuardConfig() {
				cmd.Println("WireGuard configuration not found. Please register with --wireguard flag or configure wg_private_key and wg_peer_public_key in config.")
				return
			}

			wgPrivKey, err := config.AppConfig.GetWGPrivateKey()
			if err != nil {
				cmd.Printf("Failed to get WireGuard private key: %v\n", err)
				return
			}

			wgPeerPubKey, err := config.AppConfig.GetWGPeerPublicKey()
			if err != nil {
				cmd.Printf("Failed to get WireGuard peer public key: %v\n", err)
				return
			}

			wgEndpointV4 := config.AppConfig.WGEndpointV4
			if wgEndpointV4 == "" {
				wgEndpointV4 = internal.WGDefaultEndpointV4
			}

			wgTunnel, err := api.CreateWireGuardTunnel(api.WireGuardConfig{
				PrivateKey:    wgPrivKey,
				PeerPublicKey: wgPeerPubKey,
				EndpointV4:    wgEndpointV4,
				EndpointV6:    config.AppConfig.WGEndpointV6,
				LocalIPv4:     config.AppConfig.IPv4,
				LocalIPv6:     config.AppConfig.IPv6,
				MTU:           mtu,
				KeepAlive:     keepalivePeriod,
			})
			if err != nil {
				cmd.Printf("Failed to create WireGuard tunnel: %v\n", err)
				return
			}
			defer wgTunnel.Close()

			tunNet = wgTunnel.Net
			log.Println("WireGuard tunnel established")
		} else {
			tunDev, tn, err := netstack.CreateNetTUN(localAddresses, dnsAddrs, mtu)
			if err != nil {
			...
			resolver := internal.GetProxyResolver(localDNS, systemDNS, tn, dnsAddrs, dnsTimeout)
		...
		server := &http.Server{
			...
			Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodConnect {
					handleHTTPSConnect(w, r, tunNet, nil)
				} else {
					handleHTTPProxy(w, r, tunNet, nil)
				}
			}),
		}
...
func init() {
	...
	httpProxyCmd.Flags().Bool("wireguard", false, "Use WireGuard protocol instead of MASQUE for the tunnel connection")
