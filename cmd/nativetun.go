	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if useWireGuard {
			if !config.AppConfig.HasWireGuardConfig() {
				log.Fatal("WireGuard configuration not found. Please register with --wireguard flag or configure wg_private_key and wg_peer_public_key in config.")
			}

			wgPrivKey, err := config.AppConfig.GetWGPrivateKey()
			if err != nil {
				log.Fatalf("Failed to get WireGuard private key: %v", err)
			}

			wgPeerPubKey, err := config.AppConfig.GetWGPeerPublicKey()
			if err != nil {
				log.Fatalf("Failed to get WireGuard peer public key: %v", err)
			}
			...
			if err != nil {
				log.Fatalf("Failed to create WireGuard tunnel: %v", err)
			}
			...
			go api.MaintainTunnel(context.Background(), api.MaintainTunnelConfig{
				...
				Endpoint:          endpoint,
				Device:            api.NewWireGuardTunnelAdapter(wgTunnel),
				MTU:               mtu,
...
func init() {
	...
	nativeTunCmd.Flags().Bool("wireguard", false, "Use WireGuard protocol instead of MASQUE for the tunnel connection")
