...
# ─────────────────────────────────────────────
# Configuration (FIXED FOR ONLINE CURL RUNS)
# ─────────────────────────────────────────────
# If script is piped via curl, BASH_SOURCE contains /proc/self/fd or /dev/fd
if [[ "${BASH_SOURCE[0]}" == *"/fd/"* || "${BASH_SOURCE[0]}" == "dev/fd"* ]]; then
    SCRIPT_DIR="$(pwd)"
else
...
WARP_IPV4_PREFIXES=(
    "188.114.96" "188.114.97" "188.114.98" "188.114.99" "162.159.192" 
    "162.159.193" "162.159.195" "8.34.146" "8.39.214" "8.39.204" 
...
print_header() {
    ...
    echo -e "${CYAN}║   ${WHITE}╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═════╝ ${CYAN}                    ║${NC}"
    echo -e "${CYAN}║                                                                         ║${NC}"
    echo -e "${CYAN}║   ${BOLD}${MAGENTA}WARP MASQUE/WireGuard Client with Smart Scanner${NC}${CYAN}                       ║${NC}"
    echo -e "${CYAN}║                                                                         ║${NC}"
    echo -e "${CYAN}╚═════════════════════════════════════════════════════════════════════════╝${NC}"
...
check_dependencies() {
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        ...
        echo -e "\n Install them with:"
        echo -e "   ${CYAN}# Termux / Debian / Ubuntu${NC}"
        echo -e "   pkg install ${missing[*]}   # or apt install"
        return 1
    fi
    return 0
}
...
check_kolwarp() {
    if [[ -f "$KOLWARP_BIN" ]] && [[ -x "$KOLWARP_BIN" ]]; then
        return 0
    fi
...
download_kolwarp() {
    ...
    if curl -L -# -o /tmp/kolwarp.tar.gz "$download_url"; then
        ...
        # If extracted name contains arch, fix it
        if [[ -f "${SCRIPT_DIR}/kolwarp-${os_type}-${arch}" ]]; then
            mv "${SCRIPT_DIR}/kolwarp-${os_type}-${arch}" "$KOLWARP_BIN"
        fi
...
do_register() {
    ...
    local cmd="$KOLWARP_BIN register -c \"$CONFIG_FILE\" --accept-tos"
    if [[ -n "$device_name" ]]; then cmd="$cmd -n \"$device_name\""; fi
    if [[ "$reg_type" == "2" ]]; then
    ...
    eval "$cmd"
    if [[ $? -eq 0 ]]; then print_success "Registration successful!"; else print_error "Registration failed"; fi
    echo -e "\n  ${DIM}Press Enter to continue...${NC}"
    read -r
}
...
generate_endpoints() {
    ...
    while [[ ${#endpoints[@]} -lt $count ]]; do
        ...
        for e in "${seen[@]}"; do
            if [[ "$e" == "$endpoint" ]]; then duplicate=true; break; fi
        done
...
test_endpoint() {
    ...
    if timeout "$timeout" bash -c "echo >/dev/tcp/$host/$port" 2>/dev/null; then
        ...
        local curl_result=$(curl -s -o /dev/null -w "%{time_total}" --connect-timeout "$timeout" --max-time "$timeout" "https://${endpoint}/" 2>/dev/null || echo "0")
...
show_scan_results() {
    ...
    if [[ $total_results -gt 20 ]]; then echo -e "  ${DIM}... and $((total_results - 20)) more endpoints${NC}"; fi
    ...
    if [[ "$choice" =~ ^[0-9]+$ ]] && [[ "$choice" -ge 1 ]] && [[ "$choice" -le "$total_results" ]]; then
        ...
        case "$action_choice" in
            1) do_connect_with_endpoint "$selected_endpoint" ;;
            2) print_success "Endpoint saved to config" ;;
...
apply_endpoint_to_config() {
    ...
    if [[ ! -f "$CONFIG_FILE" ]]; then print_error "Config file not found. Please register first."; return 1; fi
    local port="${full_endpoint##*:}"
    print_info "Applying endpoint to config..."
    if command -v jq &>/dev/null; then
        local temp_file=$(mktemp)
        jq --arg ep "$host" --arg port "$port" '.endpoint_v4 = $ep' "$CONFIG_FILE" > "$temp_file"
...
do_connect_with_endpoint() {
    ...
    case "$mode_cmd" in
        socks|http-proxy|l4-socks|l4-http-proxy)
            ...
            case "$mode_cmd" in
                socks|l4-socks) port="1080" ;;
                http-proxy|l4-http-proxy) port="8000" ;;
            esac
    ...
    local cmd="$KOLWARP_BIN $mode_cmd -c \"$CONFIG_FILE\" $protocol_args"
    if [[ -n "$bind_addr" && -n "$port" ]]; then cmd="$cmd -b $bind_addr -p $port"; fi
    cmd="$cmd $extra_args"
    ...
    if [[ -n "$bind_addr" && -n "$port" ]]; then echo -e "  ${CYAN}Listen:${NC}   ${BOLD}${bind_addr}:${port}${NC}"; fi
    echo -e "\n${LINE}"
...
# ─────────────────────────────────────────────
do_quick_connect() {
    if [[ ! -f "$KOLWARP_BIN" ]]; then print_error "kolwarp binary not found at ${KOLWARP_BIN}"; return; fi
    if [[ ! -f "$CONFIG_FILE" ]]; then print_error "No config found. Please register first."; return; fi
...
# ─────────────────────────────────────────────
do_account_info() {
    if [[ ! -f "$KOLWARP_BIN" ]]; then print_error "kolwarp binary not found"; return; fi
    if [[ ! -f "$CONFIG_FILE" ]]; then print_error "No config found"; return; fi
    print_section "Account Information"
    if command -v jq &>/dev/null; then
        echo -e "  ${BOLD}Device ID:${NC}     $(jq -r '.id // "N/A"' "$CONFIG_FILE")"
        echo -e "  ${BOLD}Internal IPv4:${NC} $(jq -r '.ipv4 // "N/A"' "$CONFIG_FILE")"
        echo -e "  ${BOLD}Internal IPv6:${NC} $(jq -r '.ipv6 // "N/A"' "$CONFIG_FILE")"
        echo -e "  ${BOLD}Endpoint:${NC}      $(jq -r '.endpoint_v4 // "N/A"' "$CONFIG_FILE")"
    fi
...
do_settings() {
    ...
    echo -e "  ${GREEN}1.${NC} View Config\n  ${GREEN}2.${NC} Reset Config\n  ${RED}0.${NC} Back"
    read -r choice
    case "$choice" in
        1) cat "$CONFIG_FILE" 2>/dev/null || print_error "No config" ;;
        2) rm -f "$CONFIG_FILE" && print_success "Reset complete" ;;
    esac
    echo -e "\n  ${DIM}Press Enter...${NC}"; read -r
}
...
do_about() {
    ...
    echo -e "  ${BOLD}kolwarp${NC} - WARP MASQUE/WireGuard Client\n  ${DIM}Version: 1.0.0${NC}"
    echo -e "  ${MAGENTA}Telegram:${NC} @kolandjs1 | ${BLUE}GitHub:${NC} github.com/kolandone"
    echo -e "\n  ${DIM}Press Enter...${NC}"; read -r
}
