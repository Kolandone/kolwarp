...
# ─────────────────────────────────────────────
# بررسی اینکه آیا اسکریپت از طریق پایپ curl اجرا شده است یا به صورت محلی
...
do_register() {
    ...
    echo -e "  ${GREEN}1.${NC} Personal WARP (MASQUE)"
    echo -e "  ${GREEN}2.${NC} Personal WARP (WireGuard)"
    echo -e "  ${GREEN}3.${NC} ZeroTrust (requires JWT token)"
    echo -e "  ${DIM}Select [1-3]:${NC} "
    read -r reg_type
    ...
    case "$mode_cmd" in
        socks|http-proxy|l4-socks|l4-http-proxy)
            ...
            if [[ "$auth_choice" =~ ^[Yy]$ ]]; then
                ...
                read -rs password
                echo
                # FIXED BUG: Removed 'echo' from the start of the variable assignment
...
do_settings() {
    ...
    case "$setting_choice" in
        1) if [[ -f "$CONFIG_FILE" ]]; then
               echo -e "\n${CYAN}Current Configuration:${NC}\n"
               cat "$CONFIG_FILE"
           else
               print_error "No config file found"
           fi ;;
        2) echo -e "  ${YELLOW}⚠ This will delete your current config${NC}"
...
main() {
    ...
    if ! check_kolwarp; then exit 1; fi
